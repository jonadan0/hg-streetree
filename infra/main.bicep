...
module client './app/client.bicep' = {
  name: 'client'
  params: {
    name: '${abbrs.appContainerApps}client-${resourceToken}'
    location: location
    tags: tags
    identityName: '${abbrs.managedIdentityUserAssignedIdentities}client-${resourceToken}'
    applicationInsightsName: monitoring.outputs.applicationInsightsName
    containerAppsEnvironmentName: appsEnv.outputs.name
    containerRegistryName: registry.outputs.name
    exists: clientExists
    appDefinition: clientDefinition
    apiUrls: [
      server.outputs.uri
    ]
  }
...
module server './app/server.bicep' = {
  ...
  params: {
    ...
    allowedOrigins: [
      'https://${abbrs.appContainerApps}client-${resourceToken}.${appsEnv.outputs.domain}'
    ]
  }
  scope: rg
}
